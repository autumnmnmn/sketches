...
from pyt.lib.diffusion.guidance import *
from pyt.lib.diffusion.schedule import *
from pyt.lib.diffusion.sdxl_encoder import PromptEncoder
from pyt.lib.diffusion.sdxl_vae import Decoder, save_approx_decode

from pyt.lib.ode import _euler_step as euler_step
from pyt.lib.log import Timer
...
seed = lambda run, meta: 23333325 + 7
...
#combine_predictions = lambda step, run: lambda p, n: _scale(step, 3) * (n - p[0]) * (0.1 * run) + _scale(step, 3) * (n - p[1]) * (1.0 - (0.1 * run))
combine_predictions = lambda step, run: lambda p, n: _scale(step, 3) * (n - p[1]) * (0.1 * run)
...
def run(run_id):
    ...
    for step_index in range(steps):
        ...
        if save_raw(run_id, step_index):
            save_raw_latents(pred_original_sample, f"{run_dir}/{run_id}_raw_{step_index:03d}.png")
        if save_approximates(run_id, step_index):
            save_approx_decode(pred_original_sample, f"{run_dir}/{run_id}_approx_{step_index:03d}.png")
    ...
    if save_final:
        ...
        for im in images_pil:
            for n in range(len(images_pil)):
                images_pil[n].save(f"{run_dir}/{meta_id:05d}_{n}_{run_id:05d}_{step_index:05d}.png")
